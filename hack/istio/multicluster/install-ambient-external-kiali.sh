...
#
# Ambient counterpart to install-external-kiali.sh. Installs two clusters,
# one with only Kiali (cluster-1 = "mgmt") and one with only Istio in ambient
# mode (cluster-2 = "mesh"). Bookinfo is deployed on the mesh cluster
# ambient-enrolled with a waypoint.
#
...
# Only install Kiali on the mgmt cluster
# shellcheck disable=SC2034
IGNORE_HOME_CLUSTER="true"
# shellcheck disable=SC2034
# This topology does not install Keycloak, so force anonymous auth.
KIALI_AUTH_STRATEGY="anonymous"

# Ensure the Kiali Helm repo is available for deploy-kiali.sh.
...
if [ "${BOOKINFO_ENABLED}" == "true" ]; then
  ...
  source ${SCRIPT_DIR}/../install-bookinfo-demo.sh --client-exe "${CLIENT_EXE}" --istio-dir "${ISTIO_DIR}" --istio-namespace "${ISTIO_NAMESPACE}" --namespace "${BOOKINFO_NAMESPACE}" --kube-context "${CLUSTER2_CONTEXT}" --auto-injection false --waypoint true -tg
fi
